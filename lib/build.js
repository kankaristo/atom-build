...
module.exports = {
  ...
  replace: function(value) {
    ...
    var userEnv = this.cmd.env;

    function replaceEnvVariable(match, name) {
      console.log(match, name);
      console.log(userEnv[name]);
      console.log(processEnv[name]);
      if (name in userEnv) {
        // Prefer the user's environment variable
        
        if (userEnv[name].indexOf(match) != -1) {
          // Replace the process environment variable with its value
          userEnv[name] = userEnv[name].replace(match, processEnv[name] || "")
        }
        
        return userEnv[name];
      } else if (name in processEnv && processEnv[name].indexOf(match) == -1) {
        // Either nothing in userEnv, or it uses a process environment variable
        // (e.g. PATH=$PATH:...)
    ...
    do {
      value = value.replace(/\$(\w+)/g, replaceEnvVariable);
    } while (value != value.replace(/\$(\w+)/g, replaceEnvVariable));
